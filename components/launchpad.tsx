...
import { createDbcLaunch, loadDeployment, uploadLogoToIrys, verifyDeployment, type Deployment } from "@/lib/chain-client";
...
export function Launchpad({ history, price, updatedAt }: { history: OrnnHistoryPoint[]; price: number | null; updatedAt: string | null }) {
  ...
  const submit = async () => {
    if (!deployment || !address || !ready || !form.name.trim() || !form.ticker.trim()) return;
    setBusy(true); setNotice(""); setCreatedPool(null);
    try {
      let logo = "";
      if (logoFile) {
      ...
      setNotice("Your cmB200 market is live on devnet.");
  ...
  const valid = form.name.trim().length > 0 && form.ticker.trim().length > 0;
  ...
  return <div className={styles.body}>
    <section className={styles.intro}><div><span className={styles.chapter}>THE LAUNCHPAD / 01</span><h1>Launch a market<br/><em>for compute.</em></h1><p>Your token trades against cmB200, a B200 GPU-hour asset. Meteora DBC powers the curve and graduates liquidity to DAMM v2.</p></div></section>
    <div className={styles.launchGrid}>
      <section className={styles.formPanel} aria-label="Create a B200 market">
        <div className={styles.panelHead}><h2>Make your market</h2><div className={styles.dbcBadge}><span>POWERED BY</span><Image alt="Meteora" height={22} src="/brand/meteora.svg" width={95} /><strong>DBC</strong></div></div>
        ...
        <div className={styles.terms}><div><span>Pair</span><strong>YOUR TOKEN / cmB200</strong></div><div><span>Curve</span><strong>Meteora DBC → DAMM v2</strong></div><div><span>Starting market cap</span><strong>100 cmB200</strong></div><div><span>Graduation target</span><strong>1,000 cmB200</strong></div></div>
        <button className={styles.launchButton} disabled={busy || !ready || (address !== null && !valid)} onClick={() => address ? void submit() : void connect()} type="button">{busy ? "Creating market…" : !address ? "Connect wallet to launch ↗" : "Launch on Meteora DBC ↗"}</button>
        {!valid && address && <p className={styles.hint}>Add a name and ticker to enable launch.</p>}
        {(notice || walletError) && <p className={styles.notice} role="status">{notice || walletError} {createdPool && <Link href={`/markets/${createdPool}`}>View market ↗</Link>}</p>}
      </section>
      <aside className={styles.previewPanel} aria-label="Live market preview">
        ...
        <div className={styles.previewCard}>
          <div className={styles.previewBanner}><span>B200 / COMPUTE MARKET</span><span>DEVNET</span></div>
          ...
          <div className={styles.previewChart}>
            ...
            <div className={styles.previewChartFoot}><span>Actual B200 reference · refreshed every 5 min</span><span>Not your token price</span></div>
          </div>
          <div className={styles.integrationStrip} aria-label="Powered by Ornn, Meteora DBC, Solana, Irys, and Helius"><span>POWERED BY</span><div className={styles.integrationWindow}><div className={styles.integrationTrack}>{[0, 1].map((copy) => <div aria-hidden={copy === 1} className={styles.integrationGroup} key={copy}><b>ORNN</b><b>METEORA DBC</b><b>SOLANA</b><b>IRYS</b><b>HELIUS</b></div>)}</div></div></div>
          ...
          <div className={styles.previewStats}><div><span>BASE ASSET</span><strong>${form.ticker || "TICKER"}</strong></div><div><span>QUOTE ASSET</span><strong>cmB200</strong></div><div><span>NETWORK</span><strong>Solana devnet</strong></div></div>
          {previewWebsite && <a className={styles.previewSite} href={previewWebsite} rel="noreferrer" target="_blank">{previewWebsite} ↗</a>}
        </div>
        <div className={styles.meteora}><div><Image alt="Meteora" height={36} src="/brand/meteora.svg" width={150} /></div><p>Powered by Meteora Dynamic Bonding Curve. CX sets the curve and fee split for this devnet launch. Liquidity migrates to DAMM v2 at graduation.</p></div>
      </aside>
    </div>
  </div>;
}
