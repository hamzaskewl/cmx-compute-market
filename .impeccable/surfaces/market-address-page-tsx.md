...
STORY: The shared site navigation keeps Market, Explore, Launchpad, and Docs accessible; the compact token strip establishes identity and cmB200 pairing. The first desktop viewport includes transaction-derived candlesticks, the buy/sell ticket, and visible trade rows. OHLC values appear on request. The final curve buy may partially fill, unused cmB200 remains in the wallet, and completed curves expose an explicit DAMM v2 migration action. The same market page then charts and trades the graduated pool.
...
FIRST VIEWPORT: Shared navigation, compact market identity strip, standard green/red candle chart with 1s, 3s, 5s, and 1m aggregation controls, narrow trade rail, and transaction tape beginning below the chart. Keep provider and commitment detail out of persistent production chrome; use concise market or recovery copy only when it changes what the trader can do. Sparse markets show a precise empty state rather than fabricated candles.
...
- When a curve reaches its exact threshold, expose a permissionless DAMM v2 migration action as a fallback to Meteora's migrator service. Once migrated, trade through the derived DAMM v2 pool on the same market page with the SDK's exact-input quote and slippage guard.
