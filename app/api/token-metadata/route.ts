...
export async function GET(request: Request) {
  ...
  const name = clean(url.searchParams.get("name"), 32) || "CMX B200 Launch";
  const symbol = clean(url.searchParams.get("symbol"), 8).toUpperCase() || "B200";
  const description = clean(url.searchParams.get("description"), 72)
  ...
  return Response.json({
    ...
    symbol,
    description,
    image: cleanImage(url.searchParams.get("image"), fallbackImage),
    attributes: [
