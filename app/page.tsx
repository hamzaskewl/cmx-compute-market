...
export default async function Home() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.marketHero} id="top">
      <div className={styles.heroLead}><div><span className={styles.kicker}>THE COMPUTE EXCHANGE / {network === "devnet" ? "DEVNET" : "MAINNET"}</span><h1>The market for <em>GPU-hours.</em></h1></div><Link href="/launch">Launch a market <span>↗</span></Link></div>
      <B200Chart history={history} price={b200?.price ?? null} />
      <div className={styles.heroBottom}><span><i /> B200 IS LIVE ON {network === "devnet" ? "DEVNET" : "MAINNET"}</span><p>{network === "devnet" ? "Follow the B200 GPU-hour price. Mint cmB200 with test USDC, then use it to trade launched tokens." : "Follow the B200 GPU-hour price and trade markets paired with cmB200."}</p><a href="#trade">Explore B200 <span>↓</span></a></div>
    </section>
    <PoweredBySlider />
    <B200Trade />
    <section className={styles.thesis} aria-labelledby="thesis-title">
      ...
      <p className={styles.proposal}>{network === "devnet" ? "The reinvestment step is a proposal, not an active fee route on devnet. Allocation and onchain controls must be set before mainnet." : "The reinvestment step is a proposal. Check the published market terms before treating it as an active fee route."}</p>
    </section>
    <section className={styles.marketsPreview} aria-label="Explore launched tokens"><MarketsDirectory b200Usd={b200?.price ?? null} preview /></section>
    <section className={styles.launchPromo} aria-labelledby="launch-title"><div><span className={styles.sectionIndex}>04 / BUILD ON IT</span><h2 id="launch-title">Your idea.<br/><em>A compute pair.</em></h2><p>Create a token paired with cmB200 through Meteora Dynamic Bonding Curve. Shape the market; CX handles the fixed launch configuration.</p><Link href="/launch">Open launchpad <span>↗</span></Link></div><div className={styles.launchArt}><div className={styles.artMark}><CXMark /></div><span>YOUR TOKEN</span><strong>×</strong><span>cmB200</span><div className={styles.artBars}><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/></div><small>POWERED BY METEORA DBC</small></div></section>
    <footer className={styles.footer}><div><CXMark /><span>COMPUTE EXCHANGE</span></div><nav aria-label="Footer"><Link href="/launch">Launchpad</Link><Link href="/markets">Markets</Link><Link href="/docs">Docs</Link></nav><span>Built on Solana {network === "devnet" ? "devnet" : "mainnet"} · Powered by Meteora DBC</span></footer>
  </main>;
}
